 ...
 * Shows user's current location on a Map and fetches nearby parking spaces.
 * Fallback to Leaflet (OpenStreetMap) if Google Maps API key is missing.
 */
...
import { MapContainer, TileLayer, Marker as LeafletMarker, Popup, Circle as LeafletCircle, useMap } from 'react-leaflet';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';
import { useNavigate } from 'react-router-dom';
import { X, MapPin, RefreshCw, Navigation, Car, Bike, AlertCircle, Zap, Star } from 'lucide-react';
...
function GoogleMapSection({ userCoords, radius, spaces, selectedSpace, setSelectedSpace, mapCenter, onMapLoad, navigate, onClose }) {
...
export default function NearbyMapModal({ onClose }) {
  ...
  const locateUser = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setUserCoords({ lat: 12.9716, lng: 77.5946 }); // Default Bangalore
        setStatus('success');
      },
  ...
  // Scroll sidebar to selected space
  useEffect(() => {
              ...
              <div className="p-4">
                ...
                <div className="flex flex-col gap-3">
                  {spaces.map((space) => (
                    <div 
                      key={space._id} 
                      ref={el => sidebarRefs.current[space._id] = el}
